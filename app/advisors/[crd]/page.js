...
export default function AdvisorDetails() {
  ...
  function normalizePrimaryWebsite(urls = []) {
    if (!Array.isArray(urls) || urls.length === 0) return null;

    try {
      // Prefer clean root domains
      ...
      // Fallback: strip subpaths from first URL
  ...
  const mainWebsite = normalizePrimaryWebsite(allWebsites);
